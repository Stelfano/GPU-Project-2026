...
__global__ void gemm_bf_naive_kernel(const __nv_bfloat16* __restrict__ A,
                                     const __nv_bfloat16* __restrict__ B,
                                     __nv_bfloat16* C, int M, int N, int K){

    //Conversion step in bf_16 already performed in main to reduce bandwidth waste
    int row = blockIdx.y * blockDim.y + threadIdx.y;
    int col = blockIdx.x * blockDim.x + threadIdx.x;
    if (row < M && col < N){
        __nv_bfloat16 acc = 0;
...
double gemm_cuda_timed_bf16(const __nv_bfloat16* h_A, const __nv_bfloat16* h_B, __nv_bfloat16* h_C,
                        int M, int N, int K, int n_reps) {
    size_t bytesA = static_cast<size_t>(M) * K * sizeof(__nv_bfloat16);
    size_t bytesB = static_cast<size_t>(K) * N * sizeof(__nv_bfloat16);
    size_t bytesC = static_cast<size_t>(M) * N * sizeof(__nv_bfloat16);
    ...
    // (inizializzazione del context, clock della GPU non ancora a regime)
    // che falserebbero la media se inclusi nella misura vera e propria.
    gemm_bf_naive_kernel<<<gridDim, blockDim>>>(d_A, d_B, d_C, M, N, K);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());
